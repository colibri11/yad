...
export const TRANSPORTS: TransportEndpoint[] = [
 ...
 */
export async function connectTargetFor(
  endpoint: TransportEndpoint,
): Promise<{ host: string; via: "ip" | "hostname" }> {
  if (!endpoint.connectByIp) return { host: endpoint.host, via: "hostname" };
  if (net.isIP(endpoint.host)) return { host: endpoint.host, via: "ip" };
  const ips = await dns.resolve(endpoint.host); // same call imapflow makes
  if (!ips?.length) throw new Error(`DNS resolve of ${endpoint.host} returned no records`);
...
function isNoProxy(host: string): boolean {
  ...
  for (const rawEntry of noProxy.split(",")) {
    ...
    else if ((entry.match(/:/g)?.length ?? 0) === 1) entry = entry.replace(/:\d+$/, "");
    if (!entry) continue;
 ...
 */
export function resolveProxy(targetHost?: string): string | undefined {
...
function isSocks(u: URL): boolean {
 ...
 */
function tlsWrap(raw: net.Socket, servername: string): Promise<tls.TLSSocket> {
  return new Promise((resolve, reject) => {
    ...
    tlsSocket.setTimeout(PROXY_CONNECT_TIMEOUT_MS, () =>
      fail(
        new Error(`TLS handshake to ${servername} timed out after ${PROXY_CONNECT_TIMEOUT_MS}ms`),
      ),
    );
...
function connectViaHttp(proxy: URL, host: string, port: number): Promise<net.Socket> {
  return new Promise((resolve, reject) => {
    ...
    const onTimeout = () =>
      socket.destroy(
        new Error(`Proxy CONNECT to ${host}:${port} timed out after ${PROXY_CONNECT_TIMEOUT_MS}ms`),
      );
...
async function connectViaSocks(proxy: URL, host: string, port: number): Promise<net.Socket> {
 ...
 */
export function establishRawSocket(
  proxyUrl: string,
  host: string,
  port: number,
): Promise<net.Socket> {
    ...
    dispatcher = new Agent({
      connect: (opts, callback) => {
        ...
        establishRawSocket(proxyUrl, host, port)
          .then(async (raw) => {
            if (!isHttps) {
              callback(null, raw as unknown as net.Socket);
              return;
            }
            const tlsSocket = await tlsWrap(raw, (opts.servername as string) || host);
            callback(null, tlsSocket as never);
          })
...
export function getDispatcher(targetHost?: string): Dispatcher | undefined {
  const proxyUrl = resolveProxy(targetHost);
  if (!proxyUrl) return undefined;
  return buildDispatcher(proxyUrl);
}

function hostOf(input: string | URL): string | undefined {
  try {
 ...
 */
export function getHttpsAgent(targetHost?: string): https.Agent | undefined {
  const proxyUrl = resolveProxy(targetHost);
  if (!proxyUrl) return undefined;
  ...
  (agent as unknown as { createConnection: CreateConnection }).createConnection = (
    options,
    callback,
  ) => {
...
export function proxyReport(): TransportProxyInfo[] {
...
export function proxySummary(): string {
  ...
  if (used.length === 0) return "Proxy: none (direct connections)";
  const byProxy = new Map<string, string[]>();
  for (const r of used) {
    const list = byProxy.get(r.proxy as string) ?? [];
    list.push(r.name);
    byProxy.set(r.proxy as string, list);
  }
...
function withTimeout<T extends net.Socket>(p: Promise<T>, ms: number): Promise<T> {
  return new Promise((resolve, reject) => {
    let settled = false;
...
export async function probeReachability(
  endpoint: TransportEndpoint,
  timeoutMs = PROXY_CONNECT_TIMEOUT_MS + 5_000,
  ...
  const connectVia: "ip" | "hostname" = endpoint.connectByIp ? "ip" : "hostname";
