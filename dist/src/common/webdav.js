...
export async function downloadToFile(auth, remotePath, localPath) {
 ...
 */
export async function uploadFromFile(auth, remotePath, localPath, contentType = "application/octet-stream") {
    const fs = await import("node:fs");
    ...
    return await new Promise((resolve, reject) => {
        ...
        req.on("response", (res) => {
            ...
            res.on("end", () => {
                ...
                if (sc >= 200 && sc < 300) {
                    ...
                    settle(new Error(`PUT ${remotePath} failed: ${sc} ${res.statusMessage}${body ? ` — ${body.slice(0, 200)}` : ""}`));
                }
            });
        ...
        req.setTimeout(timeoutMs, () => {
            req.destroy(new Error(`PUT ${remotePath} timed out after ${Math.round(timeoutMs / 1000)}s. ` +
...
export async function mkcolRecursive(auth, path) {
    const segments = path.split("/").filter(Boolean);
    const created = [];
    let current = "";
    for (const seg of segments) {
        current += `/${seg}`;
        if (await exists(auth, current))
            continue;
...
export async function publish(auth, path) {
...
export async function unpublish(auth, path) {
    const body = `<?xml version="1.0" encoding="utf-8" ?>
<propertyupdate xmlns="DAV:">
  <remove><prop>
