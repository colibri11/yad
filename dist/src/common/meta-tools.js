...
export function createMetaTools(info) {
    return [
        {
            name: "yad_version",
            description: "Return runtime info about this yad instance: plugin version, " +
        ...
        {
            name: "yad_diagnose",
            description: "Diagnose yad network transport, especially in fail-closed (proxy-only) " +
            ...
            async execute(_id, params) {
                ...
                if (params.probe !== false) {
                    result.reachability = await Promise.all(TRANSPORTS.map((endpoint) => probeReachability(endpoint)));
                }
