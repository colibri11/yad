...
function addressBookUrl(login) {
    return `${CARDDAV_BASE}/addressbook/${encodeURIComponent(login)}/1/`;
}
function contactUrl(login, filename) {
...
<d:propfind xmlns:d="DAV:">
  <d:prop>
    ...
    for (let m = responseRegex.exec(xml); m !== null; m = responseRegex.exec(xml)) {
        ...
        if (block.includes("<D:collection") || block.includes("<d:collection"))
            continue;
        const href = extractTag(block, "href") || "";
...
export async function fetchAllContacts(auth) {
...
export async function putContact(auth, filename, vcard, etag) {
...
export async function updateContact(auth, href, vcard, etag) {
    const url = href.startsWith("http") ? href : `${CARDDAV_BASE}${href}`;
    const headers = {
        Authorization: authHeader(auth),
        "Content-Type": "text/vcard; charset=utf-8",
