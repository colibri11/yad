...
async function getUploadHref(token, remotePath, overwrite) {
...
async function getDownloadHref(token, remotePath) {
...
async function putToUploadUrl(uploadUrl, localPath) {
    const fs = await import("node:fs");
    ...
    return await new Promise((resolve, reject) => {
        ...
        const settle = (err) => {
            ...
            if (progressPath) {
                fs.appendFileSync(progressPath, err ? `error sent=${bytesSent}: ${err.message}\n` : `done sent=${bytesSent}\n`);
            }
            if (err)
                reject(err);
            else
        ...
        req.on("response", (res) => {
            ...
            res.on("end", () => {
                ...
                if (sc >= 200 && sc < 300)
                    ...
                    settle(new Error(`Yandex REST PUT failed: ${sc} ${res.statusMessage}${body ? ` — ${body.slice(0, 200)}` : ""}`));
            });
...
async function getFromDownloadUrl(downloadUrl, localPath) {
    const fs = await import("node:fs");
...
async function getBufferFromDownloadUrl(downloadUrl) {
    const res = await fetch(downloadUrl);
    if (!res.ok) {
        throw new Error(`Yandex REST GET failed: ${res.status} ${res.statusText}`);
    }
...
export async function restUploadFile(token, remotePath, localPath, overwrite = true) {
...
export async function restDownloadToFile(token, remotePath, localPath) {
...
export async function restDownloadBuffer(token, remotePath) {
    const href = await getDownloadHref(token, remotePath);
