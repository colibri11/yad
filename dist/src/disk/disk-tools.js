...
import { isLikelyText, jsonResult, requirePassword, resolveLogin, textResult, } from "../common/types.js";
...
function diskAuth(config) {
    return {
        login: resolveLogin(config.login),
        password: requirePassword(config, "disk"),
    };
}
export function createDiskTools(config) {
    const auth = () => diskAuth(config);
    return [
        {
            name: "yad_disk_list",
            description: "List files and folders in Yandex.Disk at a given path. " +
        ...
        },
        {
            ...
            async execute(_id, params) {
                ...
                if (resources.length === 0)
                    throw new Error(`Resource not found: ${params.path}`);
                const r = resources[0];
                return jsonResult({
        ...
        },
        {
            ...
            description: "Download a file from Yandex.Disk. Two modes:\n" +
            ...
            parameters: Type.Object({
                ...
                target_path: Type.Optional(Type.String({
                    description: "Absolute local filesystem path to stream the file to. When set, content is NOT returned in the response.",
                })),
            ...
            async execute(_id, params) {
                ...
                if (params.target_path) {
                    ...
                    if (fs.existsSync(params.target_path) && !params.overwrite) {
                        throw new Error(`target_path already exists: ${params.target_path} (pass overwrite=true to replace)`);
                    }
                    fs.mkdirSync(pathMod.dirname(params.target_path), { recursive: true });
                    const { bytes, contentType } = useRest
                        ? await rest.restDownloadToFile(config.disk_oauth_token, params.path, params.target_path)
                ...
                const buf = useRest
                    ? await rest.restDownloadBuffer(config.disk_oauth_token, params.path)
        ...
        },
        {
            ...
            description: "Upload a file to Yandex.Disk. Three modes:\n" +
            ...
            parameters: Type.Object({
                ...
                content: Type.Optional(Type.String({ description: "File content: plain text or base64-encoded binary" })),
                ...
                content_type: Type.Optional(Type.String({
                    description: "MIME type, defaults to text/plain for text or application/octet-stream for binary/file",
                })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
                let buf;
                ...
                if (params.source_path) {
                    ...
                    if (useRest && !config.disk_oauth_token) {
                        throw new Error(`File ${params.source_path} is ${size} bytes — exceeds Yandex WebDAV's ~10 MB safe limit. ` +
                    ...
                    if (useRest) {
                        const { bytes } = await rest.restUploadFile(config.disk_oauth_token, params.path, params.source_path, true);
                    ...
                    const { bytes } = await webdav.uploadFromFile(auth(), params.path, params.source_path, params.content_type || "application/octet-stream");
                ...
                if (buf.length > WEBDAV_UPLOAD_LIMIT_BYTES) {
                    if (!config.disk_oauth_token) {
                        throw new Error(`Inline content is ${buf.length} bytes — exceeds Yandex WebDAV's ~10 MB safe limit. ` +
                    ...
                    try {
                        const { bytes } = await rest.restUploadFile(config.disk_oauth_token, params.path, tmpPath, true);
                        return textResult(`Uploaded to ${params.path} (${bytes} bytes, transport: rest)`);
                    }
                    finally {
                        try {
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                overwrite: Type.Optional(Type.Boolean({ description: "Overwrite if exists", default: false })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
        ...
        },
        {
            ...
            description: "Publish a file or folder in Yandex.Disk and get a public link, " +
                "or unpublish it to remove public access.",
            parameters: Type.Object({
                path: Type.String({ description: "Path to the resource" }),
                unpublish: Type.Optional(Type.Boolean({ description: "Set to true to unpublish", default: false })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
                if (params.unpublish) {
