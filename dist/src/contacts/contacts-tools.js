...
function contactsAuth(config) {
    return {
        login: resolveLogin(config.login),
        password: requirePassword(config, "contacts"),
    };
}
export function createContactsTools(config) {
    const auth = () => contactsAuth(config);
    return [
        {
            name: "yad_contacts_list",
            description: "List all contacts from Yandex.Contacts address book. " +
        ...
        },
        {
            ...
            description: "Update an existing contact in Yandex.Contacts. " +
