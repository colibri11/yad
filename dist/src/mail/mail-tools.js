 ...
 */
async function fetchOneByUid(client, uid, query) {
    // Try UID-based fetch first
    const result = await client.fetchOne(String(uid), { ...query, uid: true });
    if (result)
        return result;
...
export function createMailTools(config) {
    return [
        {
            name: "yad_mail_list",
            description: "List email messages in a Yandex.Mail folder. " +
                "Returns subject, from, date, and flags for each message.",
            parameters: Type.Object({
                folder: Type.Optional(Type.String({ description: 'Mailbox folder, default "INBOX"', default: "INBOX" })),
        ...
        {
            name: "yad_mail_read",
            description: "Read a specific email message by UID. " +
        ...
        },
        {
            ...
            parameters: Type.Object({
                ...
                }, { additionalProperties: false }), { description: "File attachments. Provide either path or content for each." })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
        ...
        },
        {
            ...
            description: "Download an email attachment by message UID and filename. " +
            ...
            parameters: Type.Object({
                ...
                index: Type.Optional(Type.Integer({
                    description: "Attachment index (0-based, as returned by yad_mail_read). Use when multiple attachments have the same filename.",
                })),
                folder: Type.Optional(Type.String({ description: 'Mailbox folder, default "INBOX"', default: "INBOX" })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
                ...
                try {
                    ...
                    try {
                        ...
                        const parsed = await simpleParser(message.source);
                        const attachment = params.index != null
        ...
        },
        {
            ...
            description: "Delete emails from a Yandex.Mail folder. DESTRUCTIVE — messages are permanently removed and cannot be recovered. " +
        ...
        {
            name: "yad_mail_mark",
            description: "Change the read/unread status of emails in a Yandex.Mail folder. " +
        ...
        },
        {
            ...
            description: "Search for emails in Yandex.Mail using IMAP search criteria. " +
