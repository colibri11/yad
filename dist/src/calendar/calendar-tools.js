...
async function createCalDavClient(config) {
...
async function putCalendarObject(auth, calendarUrl, filename, ical) {
...
export function createCalendarTools(config) {
    return [
        {
            ...
            async execute() {
                ...
                return jsonResult(calendars.map((c) => ({
        ...
        },
        {
            ...
            description: "List events from a Yandex Calendar within an optional date range. " +
                "If no calendar URL is given, events from the default calendar are returned.",
            parameters: Type.Object({
                calendar_url: Type.Optional(Type.String({
                    description: "Calendar URL from yad_calendar_list. If omitted, uses the first calendar.",
                })),
                start: Type.Optional(Type.String({ description: "Start date/time (ISO 8601), e.g. 2026-04-01T00:00:00Z" })),
                end: Type.Optional(Type.String({ description: "End date/time (ISO 8601), e.g. 2026-04-30T23:59:59Z" })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
                const client = await createCalDavClient(config);
                let calendar;
                if (params.calendar_url) {
                    calendar = { url: params.calendar_url };
                }
                else {
                    const calendars = await client.fetchCalendars();
                ...
                const end = params.end ?? (start ? new Date(Date.now() + 30 * 86400000).toISOString() : undefined);
        ...
        },
        {
            ...
            description: "Create a new event in Yandex Calendar. " + "Specify at least a title and start time.",
            parameters: Type.Object({
                ...
                calendar_url: Type.Optional(Type.String({ description: "Calendar URL. If omitted, uses the first calendar." })),
            }, { additionalProperties: false }),
            async execute(_id, params) {
        ...
        },
        {
            ...
            description: "Update an existing event in Yandex Calendar. " +
            ...
            async execute(_id, params) {
                ...
                const calUrl = lastSlash >= 0 ? params.event_url.substring(0, lastSlash + 1) : params.event_url;
                const objects = await client.fetchCalendarObjects({
                    calendar: { url: calUrl },
                });
                const existing = objects.find((o) => o.url === params.event_url);
                if (!existing?.data)
                    throw new Error(`Event not found at ${params.event_url}`);
