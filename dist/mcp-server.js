...
function loadPkg() {
    ...
    throw new Error("yad-mcp: package.json not found");
}
const pkg = loadPkg();
// ---------------------------------------------------------------------------
...
if (config.disk_app_password) {
    tools.push(...createDiskTools(config));
    enabled.push("Disk");
}
if (config.mail_app_password) {
    tools.push(...createMailTools(config));
    enabled.push("Mail");
}
if (config.calendar_app_password) {
    tools.push(...createCalendarTools(config));
    enabled.push("Calendar");
}
if (config.contacts_app_password) {
    tools.push(...createContactsTools(config));
    enabled.push("Contacts");
}
if (tools.length === 0) {
    console.error("yad-mcp: no app passwords configured. " +
...
async function handleMessage(line) {
    ...
    if (msg.jsonrpc !== "2.0" || typeof msg.method !== "string") {
        const id = isValidId(msg.id) ? msg.id : null;
